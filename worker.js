const CONTACT_TO = "kontakt@defuturum.com";
const CONTACT_FROM = "formularz@defuturum.com";
...
// Leftover URLs from the old WordPress site that Google may have crawled.
// Redirect them instead of 404ing so any inbound links/link equity land
...
async function handleContact(request, env) {
  ...
  const html = `
    <h2>Nowe zapytanie ze strony defuturum.com</h2>
  ...
  try {
    const res = await fetch(RESEND_API_URL, {
      ...
      body: JSON.stringify({
        from: `Formularz DE FUTURUM <${CONTACT_FROM}>`,
        to: [CONTACT_TO],
        reply_to: email,
